...
export function generateUniqueAgeScript() {
  ...
  (isGen ? ageGenMap : ageMap).forEach(({ label, ages }) => {
    ...
    patterns.forEach((category) => {
      const populationSum = ages
        .map(
          (age) =>
            `doc['${category}_MALE_${age}_POPUL_NUM'].size() > 0 ? doc['${category}_MALE_${age}_POPUL_NUM'].value : 0.0` +
            ` + doc['${category}_FEML_${age}_POPUL_NUM'].size() > 0 ? doc['${category}_FEML_${age}_POPUL_NUM'].value : 0.0`
        )
        .join(" + ");
...
export function generateAgeAbbMapScript(isGen: boolean = true): string {
  // 초기화 코드
  let script = `
    if (state.age_groups == null) {
      state.age_groups = [:];
  ...
  (isGen ? ageGenMap : ageMap).forEach(({ label, ages }) => {
    ...
    const populationSum = ages
      .map(
        (age) =>
          `doc['M${age}'].size() > 0 ? doc['M${age}'].value : 0.0` +
          ` + doc['F${age}'].size() > 0 ? doc['F${age}'].value : 0.0`
      )
      .join(" + ");
...
export function generatePatternSexAgeSumMapScript(
  ...
  ages.forEach(({ label, ages }) => {
    selectedPatterns.forEach((category) =>
      sex.forEach((gender) => {
        const fields =
          ages
...
export function generateNativeSumAvgScript(
  type: string,
  selectedSex: string[],
  selectedPatterns: string[],
  selectedAges: string[],
  ...
  selectedAges.forEach((ageGroup) => {
    ...
    // 선택된 패턴과 성별에 따라 스크립트 생성
    selectedPatterns.forEach((pattern) => {
      ages[ageGroup].forEach((age) => {
        selectedSex.forEach((sex) => {
          scriptParts.push(`doc['${pattern}_${sex}_${age}_POPUL_NUM'].value`);
        });
      });
    });
  });
